...
const applyMode = (mode) => {
  ...
  if (modeToggle) {
    ...
    modeToggle.textContent = basic ? 'Advanced mode' : 'Basic mode';
    modeToggle.setAttribute('aria-pressed', basic ? 'true' : 'false');
  }
  localStorage.setItem('pdmUiMode', mode);
};
...
if (modeToggle) {
  modeToggle.addEventListener('click', () => {
    applyMode(uiMode === 'basic' ? 'advanced' : 'basic');
  });
}
...
if (diagnosticsButton && diagnosticsOutput) {
  diagnosticsButton.addEventListener('click', async () => {
    ...
    const resourceChecks = await Promise.all([
      fetch('install.sh', { method: 'HEAD' }).then((r) => r.ok).catch(() => false),
      fetch('install.ps1', { method: 'HEAD' }).then((r) => r.ok).catch(() => false),
      fetch('vault.sql', { method: 'HEAD' }).then((r) => r.ok).catch(() => false),
      fetch('data.json', { method: 'HEAD' }).then((r) => r.ok).catch(() => false),
    ]);

    const checks = [
      { name: 'install.sh reachable', pass: resourceChecks[0] },
      { name: 'install.ps1 reachable', pass: resourceChecks[1] },
      { name: 'vault.sql reachable', pass: resourceChecks[2] },
      { name: 'data.json reachable', pass: resourceChecks[3] },
      { name: 'Local storage writable', pass: !!window.localStorage },
      { name: 'Client data loaded', pass: Array.isArray(vaultData) && vaultData.length > 0 },
    ];

    const stamp = new Date().toLocaleString();
    diagnosticsOutput.innerHTML = checks
      .map((c) => `<span class="diag-${c.pass ? 'pass' : 'fail'}">${c.pass ? '✅' : '❌'} ${c.name}</span>`)
      .join('<br/>') + `<br/><span class="diag-time">Checked: ${stamp}</span>`;
  });
}
...
document.addEventListener('click', (event) => {
  ...
  if (!(target instanceof HTMLElement) || !target.classList.contains('link-button')) {
    return;
  }
  const text = target.textContent || '';
  if (text.includes('Reset filters')) {
    vaultFilter = 'all';
    if (searchInput) searchInput.value = '';
    filterPills.forEach((pill) => {
      pill.classList.toggle('pill--active', (pill.dataset.filter || 'all') === 'all');
    });
    localStorage.setItem('pdmFilter', 'all');
    localStorage.setItem('pdmSearch', '');
    renderVault();
  }
});
...
const restoreState = () => {
  ...
  const savedMode = localStorage.getItem('pdmUiMode');
  ...
  applyMode(savedMode === 'basic' ? 'basic' : 'advanced');
